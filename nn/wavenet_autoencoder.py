...
class WavenetAutoencoder(torch.nn.Module):
    def __init__(self, wavenet_params, encoder_params, use_VAE):
        ...
        self.encoder_wavenet = Wavenet(use_skip_out=False, same_cond_each_resblock=False,**encoder_params)
        self.wavenet = Wavenet(use_skip_out=False, same_cond_each_resblock=False,**wavenet_params)
    ...
    def forward(self, forward_input, training=True):
        ...
        cond_features = self.encoder_wavenet((None, midi_features), training)
        ...
        y_preds = self.wavenet((cond_features, forward_input), training)
    ...
    def inference(self, midi_features, **kwargs):
        ...
        cond_features = self.encoder_wavenet((null_features, midi_features), training=False)
